...
def jetFakesEstimation(all_samples, cut, int_lumi, total_weight):
    ...
    for sample in all_samples:
        if (not sample.is_signal) and (not sample.is_data):
            ...
            sample.weight_expr = '('+sample.weight_expr+')'+'*getTauWeight(l1_gen_match, l1_pt, l1_eta, l1_decayMode,1)*getTauWeight(l2_gen_match, l2_pt, l2_eta, l2_decayMode,1)'
    
    not_jet_fakes_cut = '*(l1_gen_match<6 || l2_gen_match<6)*getTauWeight(l1_gen_match, l1_pt, l1_eta, l1_decayMode,0)*getTauWeight(l2_gen_match, l2_pt, l2_eta, l2_decayMode,0)'
    ...
    JetFakes_samples = copy.deepcopy([s for s in all_samples if s.name == 'data_obs'])
    ...
    JetFakes = HistogramCfg(name='jetFakes', var=None, cfgs=JetFakes_samples+to_substract, cut=str(cut), lumi=int_lumi, weight=total_weight)

    # all_samples = [x for x in all_samples if x.name in ['data_obs']]
    all_samples.append(JetFakes)
    # all_samples = [JetFakes]
    return all_samples
