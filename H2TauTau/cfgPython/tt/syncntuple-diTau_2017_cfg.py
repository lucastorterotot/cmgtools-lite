...
data_list = data_tau
samples = [sync_list[0]] # backgrounds + sm_signals + sync_list + mssm_signals 
if doSUSY:
...
if cmssw:
    if cmssw_reuse and all([os.path.isfile('preprocessed_files/'+comp.name+'/cmsswPreProcessing.root') for comp in selectedComponents]):
        ...
        prepdata = '/'.join([prepdir, 'h2TauTauMiniAOD_ditau_data_cfg.py'])
        prepmc = '/'.join([prepdir, 'h2TauTauMiniAOD_ditau_cfg.py'.format(tes_string=tes_string)])
