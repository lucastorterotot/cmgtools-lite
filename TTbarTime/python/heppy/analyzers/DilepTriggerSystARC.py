...
class DilepTriggerSystARC(Analyzer):

    def __init__(self, cfg_ana, cfg_comp, looperName):
        super(DilepTriggerSystARC, self).__init__(cfg_ana, cfg_comp, looperName)
    ...
    def process(self, event):
        ...
        if(muon.pt() <= 1200):
            if(muon.pt() >= 29):
                syst_muon_isomu27_weight *= self.mc_syst_muon_isomu27_hist.GetBinError(pt_binned_IsoMu27(muon.pt()), eta_binned(abs(muon.eta())))
            if(muon.pt() >= 52):
                syst_muon_mu50_weight    *= self.mc_syst_muon_mu50_hist.GetBinError(pt_binned_Mu50(muon.pt()), eta_binned(abs(muon.eta())))

        for dilep in dilepton:
            if(dilep.pt_lead() <= 200 and dilep.pt_sublead() <= 200): 
                syst_ee_trig_weight *= self.mc_syst_ee_trig_hist.GetBinError(pt_binned(dilep.pt_lead()), pt_binned(dilep.pt_sublead()))
                syst_em_trig_weight *= self.mc_syst_em_trig_hist.GetBinError(pt_binned(dilep.pt_lead()), pt_binned(dilep.pt_sublead()))
                syst_mm_trig_weight *= self.mc_syst_mm_trig_hist.GetBinError(pt_binned(dilep.pt_lead()), pt_binned(dilep.pt_sublead()))
...
#        setattr(event, 'systMMTrigWeight', syst_mm_trig_weight)
        ...
        setattr(event, 'systTrigIsoMu27Weight', syst_muon_isomu27_weight)
        setattr(event, 'systTrigMu50Weight', syst_muon_mu50_weight)

#        event.eventWeight *= event.systEETrigWeight
#        event.eventSystWeight *= event.systEMTrigWeight
#        event.eventWeight *= event.systMMTrigWeight
#        event.eventSystWeight *= event.systTrigIsoMu27Weight
#        event.eventSystWeight *= event.systTrigMu50Weight
