...
def harvest(src, subdir_pattern='*', tgz_pattern='*', apply_ff=False, convert_ntuple=False):
    ...
    if ds.fetch():
        ...
        if len(ds.subdirs) > 1:
            paths_to_hadd = [ds.dest+'/'+subdir+'/'+compname+'/BJetEfficiencyCreator/btag.root' for subdir in ds.subdirs]
            command = 'hadd '+ds.dest+'/btag.root'
            command = ' '.join([command]+paths_to_hadd)
            os.system(command)
        else:
            os.system('mv '+ds.dest+'/'+ds.subdirs[0]+'/'+compname+'/BJetEfficiencyCreator/btag.root '+ds.dest+'/btag.root')
        ...
        if convert_ntuple:
            ...
            convert_ntuple_cmd+= 'convert_ntuple.py '+ds.dest+'/btag.root'+" 'b' -o "+ds.dest+'/tree_converted.root'
            convert_ntuple_cmd+= ' && mv '+ds.dest+'/tree_converted.root '+ds.dest+'/btag.root '
